...
class BSTMap(object):
	...
	def _bstSearch(self, subtree, target):
		...
		elif target > subtree.key:	# target is right f the subtree root.
			return self._bstSearch(subtree.right, target)
		else:
			return subtree
			
	# Helper methd for finding the node containing the minimum key.
...
class _BSTMapIterator2(object):
	def __init__(self, root, size):
		# Create an array and fills it with the keys.
